...
def save_as_array(data_dict: dict, output_path: Path):
    ...
    with h5py.File(output_path, "w") as f:
        ...
        f.attrs["description"] = (
            "Processed TopTagging dataset with kt-clustered subjets"
        )


def sort_by_pt(chunk_results: dict, num_prongs_list: list[int]) -> dict:
    """Sort particles and subjets by pT descending within each event.

    Padded particles (pT=0) fall naturally to the end.
    """
    # Particle sorting
    part_px = chunk_results["part_px"]
    part_py = chunk_results["part_py"]
    if isinstance(part_px, ak.Array):
        part_px = ak.to_numpy(part_px)
        part_py = ak.to_numpy(part_py)
    part_pt = np.sqrt(part_px**2 + part_py**2)
    part_sort_idx = np.argsort(-part_pt, axis=1)  # descending

    for key in list(chunk_results.keys()):
        if key.startswith("part_"):
            val = chunk_results[key]
            if isinstance(val, ak.Array):
                val = ak.to_numpy(val)
            if val.ndim == 2 and val.shape[1] == part_sort_idx.shape[1]:
                chunk_results[key] = np.take_along_axis(val, part_sort_idx, axis=1)
    ...
    for num_prongs in num_prongs_list:
        ...
        px_key = f"{prefix}px"
        py_key = f"{prefix}py"
        if px_key not in chunk_results:
            continue
        sj_px = chunk_results[px_key]
        sj_py = chunk_results[py_key]
        if isinstance(sj_px, ak.Array):
            sj_px = ak.to_numpy(sj_px)
            sj_py = ak.to_numpy(sj_py)
        sj_pt = np.sqrt(sj_px**2 + sj_py**2)
        sj_sort_idx = np.argsort(-sj_pt, axis=1)
        ...
        for key in list(chunk_results.keys()):
            if key.startswith(prefix):
                ...
                if isinstance(val, ak.Array):
                    val = ak.to_numpy(val)
                if val.ndim == 2 and val.shape[1] == sj_sort_idx.shape[1]:
                    chunk_results[key] = np.take_along_axis(val, sj_sort_idx, axis=1)
...
def process_chunk(
    ...
    # Number of real particles per jet
    is_real_np = ak.to_numpy(is_real)
    jet_nparticles = is_real_np.sum(axis=1).astype(np.float64)
    ...
    # Sort particles and subjets by pT descending
    if sort_by_pt_flag:
        chunk_results = sort_by_pt(chunk_results, num_prongs_list)
...
def process_file(
    args: tuple[Path, Path, float, str, list[int], int, int, int, str, bool, int, bool],
) -> None:
    (
        input_path,
        output_dir,
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
            futures = {
                executor.submit(process_chunk, chunk, dR, algorithm, num_prongs_list, sort_by_pt_flag): i
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=True,
        dest="sort_by_pt",
        help="Sort particles and subjets by pT descending (default: True)",
    )
    parser.add_argument(
        "--no-sort-by-pt",
        action="store_false",
        dest="sort_by_pt",
        help="Disable pT sorting",
    )
    parser.add_argument(
        "--log-level",
        type=str,
