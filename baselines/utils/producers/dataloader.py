...
def get_dataloader_and_config(
    ...
    if mode not in config:
        raise ValueError(f"Mode {mode} not found in config for dataloader. Available keys: {list(config.keys())}")
    ...
    preprocessed = config_dataloader.get("preprocessed", True)
    ...
    LOGGER.warning("Forcing preprocessed=True for dataloader")
    if preprocessed:
        LOGGER.info("Using preprocessed dataloader and dataset classes.")
        if cached:
            LOGGER.debug("Using processed.CachedJetDataset")
            dataset = processed.CachedJetDataset(
                config=dataloader_config, split=data_split, accelerator=accelerator, **kwargs
            )
        else:
            LOGGER.debug("Using processed.JetDataset")
            dataset = processed.JetDataset(
                config=dataloader_config, split=data_split, **kwargs
            )
    else:
        LOGGER.info("Using raw dataloader and dataset classes.")
        if cached:
            LOGGER.debug("Using jetclass.CachedJetDataset")
            dataset = jetclass.CachedJetDataset(
                config=dataloader_config, split=data_split, accelerator=accelerator, **kwargs
            )
        else:
            LOGGER.debug("Using jetclass.JetDataset")
    ...
    if preprocessed:
        ...
        _atomic = getattr(dataset.config, "batch_size_atomic", -1)
        if _atomic is None or _atomic <= 0:
            _bsz = dataset.config.batch_size
        else:
            _bsz = dataset.config.batch_size // _atomic
