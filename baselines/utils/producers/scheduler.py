...
def _scale_scheduler_config_to_steps(scheduler_config: dict, steps_per_epoch: int) -> dict:
    ...
    if name == "sequentiallr":
        params["milestones"] = [m * steps_per_epoch for m in params.get("milestones", [])]
...
def get_scheduler(
    ...
    if steps_per_epoch is not None and scheduler_name != "reducelronplateau":
        scheduler_config = _scale_scheduler_config_to_steps(scheduler_config, steps_per_epoch)
        LOGGER.info(
