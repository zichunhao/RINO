...
def setup_logger(
    ...
    # Base message format
    message_format = "[%(name)-10s][%(asctime)s][%(levelname)-10s]  %(message)s (%(filename)s:%(lineno)d)"
    ...
    message = f"[$BOLD%(name)-10s$RESET][%(asctime)s][%(levelname)-10s]  %(message)s ($BOLD%(filename)s$RESET:%(lineno)d)"
