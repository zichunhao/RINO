...
def load_jetnet_data(
    file_path: Path, num_particles: int = 30
) -> tuple[np.ndarray, np.ndarray]:
    with h5py.File(file_path, "r") as f:
        data = {key: f[key][:] for key in f.keys()}

    part_energy = data["part_energy"]
    part_px = data["part_px"]
    part_py = data["part_py"]
    part_pz = data["part_pz"]
    p4 = np.stack([part_energy, part_px, part_py, part_pz], axis=-1)
    if p4.shape[1] < num_particles:
        padding = np.zeros((p4.shape[0], num_particles - p4.shape[1], 4))
        p4 = np.concatenate([p4, padding], axis=1)
    elif p4.shape[1] > num_particles:
        p4 = p4[:, :num_particles]
    else:
        pass

    jet_label = data["jet_label"]
...
def process_file(
    ...
    try:
        # Load data
        p4, labels = load_jetnet_data(input_path, num_particles=num_particles)
...
def main(args: argparse.Namespace) -> None:
    ...
    if not files:
        logging.warning(f"No h5 files found in {input_dir}")
        return

    logging.info(f"Found {len(files)} parquet files to process")
