...
def data_dependent_init_forward_hook(
    ...
    def sample_centroids(z_e, num_codes):
        ...
        if num_codes >= z_e.size(0):
            ...
            if use_kmeans:
                ...
                zed = z_e.data.detach().cpu().numpy()
                kmeans = KMeans(
                    n_clusters=num_codes,
                    random_state=0,
                    init="k-means++",
                    n_init="auto",
                ).fit(zed)
                new_codes = torch.Tensor(kmeans.cluster_centers_).to(z_e.data)
                # NOTE can't use this library
    ...
    # TODO This doesn't work with subclassing :(
