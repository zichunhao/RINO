...
def process_views(
    ...
    seq_len : int
        The default sequence length to pad to (can be overridden per-view)
    ...
    for view_name, view_config in config.outputs.views.items():
        # Determine view-specific seq_len at the start
        ...
        for feature in view_config["features"]:
            ...
            if isinstance(arr, ak.Array):
                # Pad the array to view_seq_len
                arr = ak.pad_none(arr, target=view_seq_len, clip=True)
                arr = arr.to_numpy()
        ...
        if isinstance(mask, ak.Array):
            # Pad the mask to view_seq_len
            mask = ak.pad_none(mask, target=view_seq_len, clip=True) + 0.0
...
def _np_to_torch(x):
    """Convert a numpy array to a torch tensor; pass through torch tensors
    and anything non-array (ak.Array, scalars, None) unchanged. Accelerate's
    dispatch_batches path broadcasts via `_gpu_broadcast_one` which rejects
    numpy arrays, so every numeric field in a yielded Batch must be torch.
    """
...
def _batch_to_torch(batch: Batch) -> Batch:
    """Non-mutating numpy→torch conversion for a Batch TypedDict.
    Returns a new Batch dict; does not modify the input in place.
    """
    out: Batch = {
        "sequence": _np_to_torch(batch["sequence"]),
        "mask": _np_to_torch(batch["mask"]),
        "class_": _np_to_torch(batch["class_"]),
        "aux": {k: _np_to_torch(v) for k, v in (batch.get("aux") or {}).items()},
        "views": {},
    }
    if batch.get("views"):
        for view_name, view_data in batch["views"].items():
            out["views"][view_name] = {
                k: _np_to_torch(v) for k, v in view_data.items()
            }
    return out


def maybe_concat_batch_slices(batch_slices: Sequence[Batch]):
    """Concatenate the batch slices if there are more than one, otherwise return
    the only batch slice. Operates on numpy arrays throughout. Torch conversion
    happens separately in `_torch_collate_fn` (the DataLoader's collate_fn).
    """
...
def _torch_collate_fn(batch_slices):
    """DataLoader collate_fn: concatenate slices (numpy ops), then convert to
    torch at the very end. Using this as the DataLoader's collate_fn keeps
    all intermediate ops in numpy (cheap, matches downstream semantics) and
    guarantees the yielded batch is torch tensors (required by accelerate's
    `_gpu_broadcast_one` in dispatch_batches=True mode).
    """
    return _batch_to_torch(maybe_concat_batch_slices(batch_slices))
...
JetDataLoader = partial(DataLoader, collate_fn=_torch_collate_fn)
