...
class AssembledModel(torch.nn.Module):
    ...
    def __init__(
        ...
        heads: (
            torch.nn.Module
            | list[torch.nn.Module | tuple[torch.nn.Module, bool]]
            | None
        ) = None,
    ):
        super().__init__()
        ...
        if heads is None:
            ...
            for i, head in enumerate(heads):
                if isinstance(head, tuple):
                    ...
                    LOGGER.info(
                        f"Head {i} provided without on_particles flag; defaulting to False."
                    )
    ...
    def forward(
        ...
        if cat_mean_pool:
            if mask is None:
                mean_pooled = particles.mean(dim=1)
            else:
                mean_pooled = (particles * mask.unsqueeze(-1)).sum(dim=1) / mask.sum(
                    dim=1, keepdim=True
                )
            rep = torch.cat([x, mean_pooled], dim=-1)
        else:
            rep = x
        ...
        if self.is_single_head:
            if getattr(self.heads, "needs_tokens", False):
                return rep, self.heads(tokens=particles, mask=mask)
            return rep, self.heads(x)
        return rep, [head(x) for head in self.heads]
