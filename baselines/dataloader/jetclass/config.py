...
class _DataloaderConfig:
    ...
    # batch_size_atomic <= 0 means "no atomic sub-batching" — the outer
    # DataLoader batch_size equals batch_size directly.
    batch_size_atomic: int = -1

    def update(self, updates: dict): ...

    def __post_init__(self):
        if self.batch_size_atomic > 0:
            assert (
