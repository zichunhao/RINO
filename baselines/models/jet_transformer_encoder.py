...
class LayerScale(nn.Module):
    """Per-channel learnable scale to stabilise deep residuals."""
...
class JetTransformerEncoder(nn.Module):
    ...
    Args:
        ...
        pooling: Pooling strategy. One of:
            ...
            - ``"cls_jet"`` – initialise the class token from projected jet features.
    ...
    def _pool(self, output: torch.Tensor, padding_mask: torch.Tensor) -> torch.Tensor:
        ...
        if self.pooling in ("cls_token_concat_mean", "cls_jet_concat_mean"):
            cls_rep = output[:, 0]
            # skip cls + registers to get particle tokens and their mask
            particles_out = output[:, 1 + self.num_registers :]
            particle_mask = padding_mask[:, 1 + self.num_registers :]
        ...
        if self.pooling == "last_token":
            # Use last valid particle token (skip CLS + register prefix tokens).
            # Designed for autoregressive models (e.g. OmniJet) where the last token
            # has attended to the full sequence via causal masking.
            prefix = int(self.uses_cls_token) + self.num_registers
            particles_out = output[:, prefix :]
            particle_padding = padding_mask[:, prefix :]  # True = padding
    ...
    def forward(
        ...
        coords: (
            torch.Tensor | None
        ) = None,  # unmasked particles for PE; falls back to particles
        ...
        if self.pos_encoding is not None:
            ...
            x = x + self.pos_encoding(pe_source.to(self.device))

        # Build prefix: [cls_ (optional)] [registers (optional)]
        prefix_parts = []
        ...
        # Separate cls_ token from particle tokens
        if self.uses_cls_token:
            particles_out = x[:, 1 + self.num_registers :]
        else:
            particles_out = x
