...
class _DataloaderConfig:
    ...
    train_fraction: float = 1.0  # Label efficiency: fraction of training data to use
    train_fraction_seed: int = 42  # Seed for subsampling (42 + run_index for per-run variance)
    transformations: list[TransformationSchema] = field(default_factory=list)
    outputs: OutputSchema = field(
        default_factory=lambda: OutputSchema(
            sequence=[], sequence_mask="", class_=[], aux=[]
        )
    )
