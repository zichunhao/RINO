...
class PointCloudDataModule(LightningDataModule):
    def __init__(
        ...
        *,
        ...
        # Load a mini dataset to infer the dimensions.  Using the val split
        # (not test) so we don't need the test h5 file local-copied just for
        # dim inference.
        mini_conf = deepcopy(self.hparams.data_conf)
        mini_conf["n_jets"] = 5
        self.mini_set = JetData(dset="val", **mini_conf)
    ...
    def get_dims(self) -> tuple:
        ...
        sample = next(self.mini_set.iterator)
        edges, nodes, high = sample[0], sample[1], sample[2]
