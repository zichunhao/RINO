...
def _scale_scheduler_config_to_steps(
    scheduler_config: dict, steps_per_epoch: int
) -> dict:
    """Return a deep copy of *scheduler_config* with all epoch counts scaled to steps.

    Handles ``CosineScheduler`` (``total_iters``) and ``SequentialLR``
    (``milestones`` + each sub-scheduler's ``total_iters``).
    """
    ...
    if name == "sequentiallr":
        params["milestones"] = [
            m * steps_per_epoch for m in params.get("milestones", [])
        ]
        LOGGER.debug(f"SequentialLR milestones (steps): {params['milestones']}")
        for i, sub in enumerate(params.get("schedulers", [])):
            sub_params = sub.get("params", {})
            if "total_iters" in sub_params:
                sub_params["total_iters"] = sub_params["total_iters"] * steps_per_epoch
                LOGGER.debug(
                    f"scheduler[{i}] ({sub.get('name', '?')}) total_iters (steps): {sub_params['total_iters']}"
                )
    elif "total_iters" in params:
        params["total_iters"] = params["total_iters"] * steps_per_epoch
        LOGGER.debug(f"{name} total_iters (steps): {params['total_iters']}")
...
def get_scheduler(
    ...
    When *steps_per_epoch* is provided and the scheduler is not
    ``ReduceLROnPlateau``, all epoch-based iteration counts (``total_iters``,
    ``milestones``) are multiplied by *steps_per_epoch* so the scheduler can
    be stepped once per batch rather than once per epoch.
    """
    ...
    if steps_per_epoch is not None and scheduler_name != "reducelronplateau":
        scheduler_config = _scale_scheduler_config_to_steps(
            scheduler_config, steps_per_epoch
        )
        LOGGER.info(
            f"Scheduler epoch counts scaled to steps (steps_per_epoch={steps_per_epoch})"
        )
...
def _create_sequential_scheduler(optimizer: Optimizer, scheduler_params: dict):
    ...
    # Snapshot original LRs before any sub-scheduler init mutates them.
    # Each LambdaLR.__init__ calls step(), which writes the initial LR into
    # the optimizer.  Without this save-restore, schedulers[1].base_lrs would
    # be set from the mutated (e.g. ~1e-6 × base) value left by schedulers[0],
    # causing the decay phase to start from the wrong LR.
    ...
    for i, sched_config in enumerate(schedulers_config):
        # Restore original LRs so every sub-scheduler sees the real base LR.
    ...
    # Restore again so SequentialLR.__init__ (which calls step() on schedulers[0])
    # starts from the correct base LR.
    for g, lr in zip(optimizer.param_groups, original_lrs):
        g["lr"] = lr
...
def _load_scheduler_state(
    ...
    if isinstance(scheduler, ReduceLROnPlateau):
        ...
        LOGGER.info(
            f"Loaded scheduler state from checkpoint (last_epoch={scheduler.last_epoch})"
        )
