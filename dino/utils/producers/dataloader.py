...
def get_dataloader_and_config(
    ...
    if accelerator is not None:
        ...
        if dispatch_batches:
            if accelerator.is_main_process:
                LOGGER.info(
                    f"Main process with dispatch_batches=True: using {num_workers} workers"
                )
            else:
                # Worker processes: no workers needed (they don't load data)
                num_workers = 0
                LOGGER.info(
                    f"Worker process {accelerator.process_index} with dispatch_batches=True: using 0 workers"
                )
        else:
            # Standard distributed training: reduce workers per process
            num_workers = max(1, num_workers // accelerator.num_processes)
            LOGGER.info(
                f"Standard distributed training: reduced to {num_workers} workers per process"
            )

    # For cached datasets, reduce workers further (data is in memory)
    cached = config_dataloader.get("cached", True)
    ...
    if prefetch_factor is not None:
        if num_workers > 0:
            ...
            if prefetch_factor < 1:
                LOGGER.warning(
                    f"Prefetch factor {prefetch_factor} < 1, using default (2)"
                )
                prefetch_factor = 2
            ...
            LOGGER.info(
                f"Dataloader (split={split}) prefetch factor: {prefetch_factor}"
            )
        else:
            LOGGER.warning(
    ...
    if "train" in split:
        ...
        raise ValueError(
            f"Unrecognized split: {split}. Split must contain 'train', 'val', or 'test'."
        )
    ...
    if preprocessed:
        _loader_cls = (
            processed.CachedJetDataLoader if cached else processed.JetDataLoader
        )
        LOGGER.info(f"Using dataloader class: {_loader_cls.__name__}")
        dataloader = _loader_cls(
            dataset,
            shuffle=shuffle,
            pin_memory=True,
            **dataloader_kwargs,
        ...
        LOGGER.info(
            f"Using dataloader class: {getattr(jetclass.JetDataLoader, '__name__', repr(jetclass.JetDataLoader))}"
        )
...
def get_config(
    ...
    try:
        ...
        LOGGER.debug(f"No dataloader config found for split {split}. Using default.")
