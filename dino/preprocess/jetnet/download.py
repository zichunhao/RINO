...
def get_jetclass_format(
    particle_data: np.ndarray, jet_data: np.ndarray
) -> dict[str, np.ndarray]:
...
def process_single_jettype(
    ...
    particle_data, jet_data = get_jetnet_dataset(
        download_dir=download_dir, jet_type=jet_type, num_particles=num_particles
    )
    ...
    val_indices = shuffle_indices[
        int(split_train * njets) : int((split_train + split_val) * njets)
    ]
    ...
    logging.info(
        f"Dataset split for {jet_type} - train={len(train_indices)}, val={len(val_indices)}, test={len(test_indices)}"
    )
...
def combine_and_shuffle_splits(
    ...
    for split in ["train", "val", "test"]:
        ...
        logging.info(
            f"Combining {len(jet_types)} jet types for {split} split ({total_samples} samples total)"
        )
...
def process_jetnet_dataset(
    ...
    logging.info(
        f"Split into train:validation:test = {splits[0]}:{splits[1]}:{splits[2]}."
    )
