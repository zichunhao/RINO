...
JET_FEATURE_LABELS = ["nparticles", "energy", "pt", "eta", "phi"]
...
def untransform_jets(jets):
    norm_nparticles, norm_log_energy, norm_log_pt, norm_eta, norm_phi = jets.unbind(
        axis=-1
    )
...
def create_kinematic_mask(
    data, jet_pt_bounds: tuple[float, float] = (500, 650)
) -> torch.Tensor:
    jets = data["jets"]
    ...
    for data_split in data_splits:
        ...
        for data in datasets:
            if "particles" in data and "jets" in data:
                plot = True
                mask = create_kinematic_mask(
                    data, jet_pt_bounds=(jet_pt_min, jet_pt_max)
                )
            else:
                plot = False
                logging.warning(
                    f"No particles or jets found in data for {data_split}. Skipping kinematic mask creation."
                )
        ...
        for data_tag, mask in zip(valid_data_tags, kinematic_masks):
            ...
            logging.info(
                f"Fraction of jets passing kinematic cuts for {data_split} - {data_label}: {fraction_passing:.2f}"
            )

        logging.info(f"Compute Accuracy and AUC for {data_split}...")
        plt.figure(figsize=(10, 5))

        for data_tag, data, kinematic_mask in zip(
            valid_data_tags, datasets, kinematic_masks
        ):
            data_label = data_label_dict.get(data_tag.lower(), data_tag)
            ...
            if not kinematic_mask.any():
                logging.warning(
                    f"No jets left after kinematic cuts for {data_label} in {data_split}. Skipping..."
                )
                continue
            ...
            metric_dict[data_label] = {"roc_auc": roc_auc, "acc": acc}
            ...
            for i, jet_feature_name in enumerate(JET_FEATURE_LABELS):
                ...
                for data_tag, data, kinematic_mask in zip(
                    ...
                    jet_features = torch.stack(
                        [nparticles, energy, pt, eta, phi], dim=-1
                    )
                ...
                plt.tight_layout()
                plt.savefig(
                    fig_dir / f"jet_{jet_feature_name}.pdf", bbox_inches="tight"
                )
                plt.close()
            ...
            for i in range(len(FEATURE_LABELS)):
                ...
                for data_tag, data, kinematic_mask in zip(
                    ...
                    for signal_type in ("QCD", "Top"):
                        ...
                        particle_features = data["particles"][sample_mask][
                            ..., i
                        ]  # [N_selected_jets, N_particles]
                        particle_mask = data["mask"][
                            sample_mask
                        ]  # [N_selected_jets, N_particles]
                ...
                plt.tight_layout()
                plt.savefig(
                    fig_dir / f"particle_{FEATURE_LABELS[i]}.pdf", bbox_inches="tight"
                )
                plt.close()
