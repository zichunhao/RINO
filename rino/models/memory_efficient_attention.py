...
checkpointed_summarize_qkv_chunk = partial(
    checkpoint, summarize_qkv_chunk, use_reentrant=True
)
...
def memory_efficient_attention(
    ...
    summarize_qkv_fn = (
        checkpointed_summarize_qkv_chunk if needs_backwards else summarize_qkv_chunk
    )
    ...
    mask_chunks = (
        mask.split(k_bucket_size, dim=-1) if exists(mask) else ((None,) * len(k_chunks))
    )

    if exists(attn_bias):
        i, j = attn_bias.shape[-2:]
        attn_bias_chunks = attn_bias.split(q_bucket_size, dim=-2)
        attn_bias_chunks = list(
            map(lambda t: t.split(k_bucket_size, dim=-1), attn_bias_chunks)
        )
    ...
    for q_index, q_chunk in enumerate(q_chunks):
        ...
        for k_index, (k_chunk, v_chunk, mask_chunk) in enumerate(
            ...
            attn_bias_chunk = (
                attn_bias_chunks[q_index][k_index] if exists(attn_bias) else None
            )
...
# wrap up and write a module similar to pytorch's MultiheadAttention
class MemoryEfficientMultiheadAttention(nn.Module):

    def __init__(self, embed_dim, num_heads, dropout=0.0, add_bias_kv=False):

        assert add_bias_kv == False, "Only add_bias_kv=False is supported"
        super().__init__()
    ...
    def forward(self, query, key, value, key_padding_mask=None, attn_mask=None, **ignore):

        assert id(key) == id(value), "key and value must be the same!"
        q = rearrange(query, "n b d -> b n d")
        k = rearrange(key, "n b d -> b n d")
        mask = (
            ~key_padding_mask
        )  # masked particle is True in memory effiecient attention implementation
        if exists(attn_mask):
            attn_mask = rearrange(attn_mask, "(b h) i j -> b h i j", b=q.shape[0])

        output = self.attn(q, context=k, mask=mask, attn_bias=attn_mask)
        return rearrange(output, "b n d -> n b d"), None
