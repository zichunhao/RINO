...
class Bert(LightningModule):
    ...
    def __init__(
        ...
        *,
        ...
        # Build encoder: either JetTransformerEncoder (via adapter) or original
        if backbone_config is not None:
            from src.models.jte_adapter import build_jte_adapter
            self.encoder = build_jte_adapter(
                backbone_config, inpt_dim=self.node_dim, outp_dim=self.n_classes
            )
        else:
    ...
    def sample_mask(self, edges, nodes, mask):
        ...
        # All tensors are created directly on GPU to avoid CPU→GPU `.to()`
        # transfers that trigger implicit CUDA syncs. Under DDP, those syncs
        # cause per-rank timing drift → NCCL allreduce deadlock.
        dev = nodes.device
        with T.no_grad():
            indx = T.randn(mask.shape, device=dev)
            indx[~mask] = -100
            K = int(self.max_mask * nodes.shape[1])
            indx = T.topk(indx, K).indices
            count_mask = F.embedding(mask.sum(-1) - 1, self.count_mask)
            valid_topk = count_mask[:, :K].to(nodes.dtype)
            false_mask = T.zeros_like(mask, dtype=nodes.dtype)
            false_mask.scatter_(1, indx, valid_topk)
    ...
    def monitor_quality(self, output, mask, og_labels):
        ...
        prediction = self.cf_monitor(output, mask)
        # og_labels is passed via the `sample[0] = sample[-1]` hack in
        # preprocess_inputs, so it's the raw float32 label from the dataloader.
        # track_loss (CrossEntropyLoss) requires long target class indices.
        og_lbl_long = og_labels.view(-1).long()
        loss = self.track_loss(prediction, og_lbl_long)
        metrics = {}
        for key, func in self.track_metrics.items():
            metrics[key] = func(prediction, og_lbl_long)
        return loss, metrics
    ...
    def _shared_step(self, sample: tuple) -> T.Tensor:
        ...
        # monitor_quality is logging-only — run it in no_grad so none of
        # cf_monitor's params participate in the backward pass.
        with T.no_grad():
            l1, m2 = self.monitor_quality(output.detach(), mask, edges)
        metrics.update(m2)

        # Anchor EVERY trainable parameter into the DDP graph with a
        # zero-weight term. Under plain DDP (`find_unused_parameters=False`),
        # every param in `self.parameters()` must have a gradient produced by
        # the backward pass, or DDP raises:
        #   "It looks like your LightningModule has parameters that were not
        #    used in producing the loss returned by training_step."
        #
        # Bert has several submodules whose params can look "unused" on a
        # given step — cf_monitor (called in no_grad), NewIterativeNormLayer
        # γ/β (frozen during init phase), conditional pos_encoding /
        # positional_encoder, etc. Rather than enumerate them, we anchor
        # `self.parameters()` as a catch-all; frozen submodules like the
        # VQ-VAE are filtered by `requires_grad`. Zero gradient impact.
        #
        # This matches the OmniJet-α DDP surface that already works, and
        # replaces the previous need for `ddp_find_unused_parameters_true`.
        anchor = sum(
...
class IterableBert(Bert):
    ...
    def preprocess_inputs(self, sample):
        with T.no_grad():
            # RINOIteratorWrapper yields a 7-tuple when precomputed
            # code_labels are available (from *_tokens.h5), else a 6-tuple.
            # Detect and extract. Truncate sample back to 6 elements so
            # downstream _shared_step's unpacking still works.
            precomputed_code_labels = None
            if len(sample) == 7:
                precomputed_code_labels = sample[6]
                # Drop the 7th element so _shared_step sees the standard 6-tuple.
                if isinstance(sample, list):
                    del sample[6]
                else:
                    sample = list(sample[:6])

            edges, nodes, high, adjmat, mask, label = sample
            # Always pass an empty tensor for the edges and high
            # Edges is no longer used, and this is an easy if hacky way to get at the label
            sample[0] = label
            if precomputed_code_labels is not None:
                # Offline tokens path — skip the runtime VQ tokenize() entirely.
