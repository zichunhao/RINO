...
class CachedJetDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        LOGGER.debug(f"Initializing CachedJetDataset (split={split}) with cache_size={cache_size}GB, "
                    f"size_multiplier={size_multiplier}, preload_workers={self.preload_workers}, "
                    f"is_main_process={self.is_main_process}, process_index={self.process_index}, num_processes={self.num_processes}")
        ...
        LOGGER.debug(f"Dataset paths for split '{split}': {self.paths}")
        ...
        LOGGER.debug(f"Dataset file sizes (number of samples): {self.sizes}")
        self.batch_indices = self._init_batch_indices(config, self.paths, self.sizes)
        LOGGER.debug(f"Initialized batch indices: {self.batch_indices[:5]}... (total {len(self.batch_indices)} batches)")
    ...
    def _cache_dataset_sizes(
        ...
        for i, p in enumerate(paths):
            with h5py.File(p, "r") as f:
                ...
                LOGGER.debug(f"File {p} has {sizes[i]} samples")
        return sizes
