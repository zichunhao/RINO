...
def _batch_to_torch(obj):
    """Recursively convert numpy arrays in a (possibly nested) dict to torch
    tensors. Leaves awkward arrays and other non-array objects untouched.

    accelerate's distributed broadcast (`_gpu_broadcast_one`) only understands
    torch tensors, so we have to hand it tensors — numpy ndarrays crash
    with TypeError during `dispatch_batches=True` main-process broadcast.
    """
    if isinstance(obj, np.ndarray):
        # np arrays from numpy or h5py slice may be non-writable — copy
        # before ``from_numpy`` to avoid "not writable" warnings downstream.
        if obj.dtype == np.bool_:
            return torch.from_numpy(obj.copy())
        if obj.dtype.kind in ("O", "U", "S"):  # object / unicode / bytes → leave
            return obj
        return torch.from_numpy(obj if obj.flags.writeable else obj.copy())
    if isinstance(obj, dict):
        return {k: _batch_to_torch(v) for k, v in obj.items()}
    if isinstance(obj, (list, tuple)):
        converted = [_batch_to_torch(x) for x in obj]
        return type(obj)(converted) if isinstance(obj, tuple) else converted
    return obj


def _collate_to_torch(batch_slices):
    return _batch_to_torch(maybe_concat_batch_slices(batch_slices))


JetDataLoader = partial(DataLoader, collate_fn=_collate_to_torch)
